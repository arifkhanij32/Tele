...
const PatientDetailsPage = ({visitId, selectedOption, onClose, patientData: initialPatientData, selectedServices = [], fetchVisits, onRefundSuccess, handleSendClick, handleApproveClick, handleRejectClick, handleRefundClick }) => {
    ...
    const [selectedReferrer, setSelectedReferrer] = useState('No Referrer Added');
    const [referrerId, setReferrerId] = useState('No Referrer Added');
    ...
    const handleReferrerSelect = (referrerName) => {
        setSelectedReferrer(referrerName);  // Update the referrer name on the UI
    };
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get(`${process.env.REACT_APP_LOCALHOST}/referrer/referrals/${latestvisitID[1]}/`);
                console.log("Fetched referral data:", response.data);

                setReferrerId(response.data.id)
                console.log("reffff",referrerId,response.data.id)
               
                const referrerName = response.data.referrer_name;  // Access the referrer_name from the API response
                console.log("Referrer Name:", referrerName);
                setSelectedReferrer(referrerName); // Set referrer name in state
        ...
        fetchData();
    }, [latestvisitID]);  // Trigger whenever latestvisitID changes
    ...
    //     // setStatus(initialBalance <= 0 ? 'Paid' : 'Unpaid');
    // }, [services, discountPercentage, discountAmount, taxPercentage, totalRefund, serviceDescription]);
    // Lock balance and payment status if fully paid, and update only when necessary
useEffect(() => {
...
const handleAddPayment = (paymentData) => {
    ...
    const calculateTotal = () => {
        ...
        // return discountedPrice + taxAmount - (totalRefund || 0);
        return discountedPrice - (totalRefund || 0);
    };
    ...
    //     try {
    //         const serviceId = serviceDescription[index].id; // Assuming each service description has an `id` field
    //         const updatedServices = serviceDescription.filter((_, i) => i !== index);
    //         setServiceDescription(updatedServices);
    ...
    // };
    // const handleAddPayment = (paymentData) => {
    //     // Update the balance after adding the payment
    //     const newBalance = balance - paymentData.amount;
    //     const updatedStatus = newBalance <= 0 ? 'Paid' : 'Unpaid';
       
    //     setBalance(newBalance);
    //     setStatus(updatedStatus); // Update the payment status
   
    //     // Add the payment to the list of payments
    //     setPayments([...payments, paymentData]);
   
    //     // Save the updated totals (balance, tax, refund, etc.) to the backend
    //     handleSaveTotals(newBalance, updatedStatus);
    // };
    // const handleAddPayment = (paymentData) => {
    //     // Calculate the new balance after applying the payment
    //     const newBalance = balance - paymentData.amount;
    ...
    const handleSaveTotals = (newBalance, updatedStatus) => {
        const totalsData = {
            visit: patientData.id,  // Ensure patientData.id is correct
            sub_total: calculateSubTotal(),
            balance: newBalance < 0 ? 0 : newBalance,  // Ensure balance is locked at zero
            // tax: calculateTax(),
            refund: totalRefund,
            discount: calculateDiscount(),
            total: calculateTotal(),
    ...
    useEffect(() => {
        if (patientData && patientData.id) {
            axios.get(`${process.env.REACT_APP_LOCALHOST}/patients/api/payments/?visitId=${patientData.id}`)
                .then(response => {
                    const filteredPayments = response.data.filter(payment => payment.visit === patientData.id);
        ...
        <div className={`patient-details-page ${isAnyModalOpen ? 'background-overlay' : ''}`}>
                            ...
                            <div className="patient-details">
                                ...
                                {isAddReferrerDialogOpen && (
                                    <AddReferrerDialog
                                        ...
                                        // visit={visit}
                                        revenue ={balance}
                                        onReferrerSelect={handleReferrerSelect}  // Pass the handler to update the referrer name
                                    />
                                )}
                                    <hr />
                                </div>
                    ...
                    {(!maximizedSection || maximizedSection === 'billing-details') && (
                        <div className={`billing-details ${maximizedSection === 'billing-details' ? 'maximized' : ''}`}>
                                        ...
                                        {serviceDescription.map((service, index) => (
                                            <li key={service.id} className="service-item">
                                                ...
                                                <div className="quantity-controls fixed-quantity-controls">
                                                    ...
                                                    disabled={balance === 0 && status === 'Paid'}  // Disable when balance is 0 and status is 'Paid'
                                                    >
                                                    +
                                                    </button>
                            ...
                            <div className="net-amount">
                                <h3>Sub Total:
                                {calculateSubTotal().toFixed(2)}</h3>
                            </div>
                            <hr />
                            <div className="net-amount1">
                            ...
                            <div className="net-amount1">
                            <button onClick={openPaymentModal} disabled={balance === 0 && status === 'Paid'}>Add Payment</button></div>
                                <h3>Payments:</h3>
                                {payments.length === 0 ? (
                                    ...
                                    <div className="payments-container">
                                        {payments.map((payment, index) => (
                                            <div className="payment-item" key={index}>
                                                <span>
                                                    Payment of {payment.amount} made using {payment.payment_mode}
                                                </span>
                                            </div>
                                        ))}
                                    </div>
            ...
            {modalShow && (
                <ServiceModal
                    ...
                    selectedReferrer={selectedReferrer} // Pass the selected referrer here
